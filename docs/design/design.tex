...
\begin{itemize}
    \item Create bit stream of the read data that follows the {Input Data Structure} specifications.
        ...
        \textbf{NOTE}:  if the mode is 16 bit (fixed), then each 16bit number must be padded with 16 bits of zeros, so that the number is in the lowest part (bits 0..15 -> num, 16 -> 31 -> 0 padding.)
...
\textbf{NOTE}: if the mode is 16 bit (fixed), then each 16bit number is padded with 16 bits of zeros, so that the number is in the lowest part (bits 0..15 -> num, 16 -> 31 -> 0 padding.)
\begin{center}
